...
print("spectral stability : ", stability_0)

data['spectral clustering'] = fused_labels_0+1

for ii in range(clusters_0):
    cluster = raw_data[data['spectral clustering'] == ii+1]
    plt.scatter(cluster['X'], cluster['Y'], label = str(ii+1))
plt.legend()
plt.savefig('figs/spectral_clusters.png')
plt.show()
